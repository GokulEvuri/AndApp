%% Edited and resued for BestPrice Project
...
%% @spec create_attchment(DBServer::server_address(), Database::string(), DocumentID::string(), File::string(), ContentType::string()) -> {"ok": true, "id": "document", "rev": Rev::string()}
%%
...
delete_document({Server, ServerPort}, Database, DocID, Revision) when is_list(Server), is_integer(ServerPort) ->
    Url = build_uri(Database, binary:bin_to_list(DocID), [{"rev", Revision}]),
